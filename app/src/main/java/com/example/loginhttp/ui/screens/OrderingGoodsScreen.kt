...
fun OrderingGoodsScreen(
    internalOrders: List<OrderItem>,
    externalOrders: List<OrderItem>,
    onAddClick: () -> Unit,
    onOrderClick: (OrderItem) -> Unit,
    onOrderLongClick: (OrderItem) -> Unit,
    selectedScreen: String = "Warehouse",
    onNavigate: (String) -> Unit,
) {
    var selectedTypeTab by remember { mutableIntStateOf(0) }   // 0 = Interne, 1 = Vanjske
    val orders = if (selectedTypeTab == 0) internalOrders else externalOrders

    val synced = orders.filter { it.synced }
    val unsynced = orders.filterNot { it.synced }
        ...
        Column(
            ...
            TabRow(selectedTabIndex = selectedTypeTab) {
                Tab(
                    ...
                    Text(
                        "Interne narudžbe (${internalOrders.size})",
